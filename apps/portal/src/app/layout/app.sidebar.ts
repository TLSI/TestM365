...
export class AppSidebar implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.router.events
            ...
            .subscribe((event) => {
                this.onRouteChange((event as NavigationEnd).urlAfterRedirects);
            });
    ...
    private onRouteChange(path: string) {
        this.layoutService.layoutState.update((val) => ({
            ...val, activePath: path, overlayMenuActive: false, mobileMenuActive: false, menuHoverActive: false
        }));
    }
        ...
        if (!this.outsideClickListener) {
            this.outsideClickListener = (event: MouseEvent) => {
                if (this.isOutsideClicked(event)) {
                    this.layoutService.layoutState.update((val) => ({
                        ...val, overlayMenuActive: false, mobileMenuActive: false, menuHoverActive: false
                    }));
                }
            };
