...
export class AppConfigurator {
    ...
    primaryColors = computed<SurfacesType[]>(() => {
        ...
        colors.forEach((color) => { palettes.push({ name: color, palette: (presetPalette as any)?.[color] as SurfacesType['palette'] }); });
        return palettes;
    });
    ...
    updateColors(event: any, type: string, color: any) {
        if (type === 'primary') this.layoutService.layoutConfig.update((state) => ({ ...state, primary: color.name }));
        else if (type === 'surface') this.layoutService.layoutConfig.update((state) => ({ ...state, surface: color.name }));
        this.applyTheme(type, color);
        event.stopPropagation();
    }

    applyTheme(type: string, color: any) {
        if (type === 'primary') updatePreset(this.getPresetExt());
